...
4. [Accessing metrics](#accessing-metrics) - `wp log metrics`
5. [Managing loggers](#managing-loggers) - `wp log logger`
6. [Using logger types](#managing-loggers) - `wp log type`
7. [Managing listeners](#managing-loggers) - `wp log listener`
...
## Introduction
DecaLog is WordPress plugin that is designed to:
- capture events, traces and metrics generated by the core of WordPress, themes and plugins;
- enrich these events, traces and metrics with many details regarding their context;
- record these events, traces and metrics in its internal storage or send them to external services;
- offer views for events and traces recorded in the WordPress database.
