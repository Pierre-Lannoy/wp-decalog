...
class Date {
	...
	public static function get_positive_time_diff_from_mysql_utc($from) {
		if (strtotime($from) < time()) {
			return sprintf( __('%s ago', 'decalog'), human_time_diff(strtotime($from)));
		}
		else {
			return __('currently', 'decalog');
		}
	}
	...
	public static function get_time_diff_from_mysql_utc($from) {
		if (strtotime($from) < time()) {
			...
			return sprintf( __('in %s', 'decalog'), human_time_diff(strtotime($from)));
		}
	}
