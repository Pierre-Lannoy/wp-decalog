...
class InlineHelp {
	...
	private function set_contextual_settings_loggers() {
		...
		$content .= '<p>' . esc_html__( 'A logger is a recorder of events, traces or metrics. It can filter them (accept or refuse to record them based on settings) then store them or send them to external services.', 'decalog' );
		$content .= ' ' . esc_html__( 'You can set as many loggers as you want. All the set loggers will receive all events, traces or metrics (depending of their types) and, regarding their own settings, will enrich them and record them or not.', 'decalog' ) . '</p>';
		$content .= '<p>' . esc_html__( 'Loggers are classified in six main categories: alerting, debugging, crash analytics, events logging, monitoring and tracing. You can find details on these categories on the corresponding tabs of this help.', 'decalog' ) . '</p>';
		$tabs[]   = [
		...
		// Crash.
		$content = '<p>' . esc_html__( 'These loggers capture crashes and send reports for crash analytics purpose:', 'decalog' ) . '</p>';
		$tabs[]  = [
		...
		// Logging.
		$content = '<p>' . esc_html__( 'These loggers capture events for logging purpose:', 'decalog' ) . '</p>';
		$tabs[]  = [
			'title'   => esc_html__( 'Events Logging', 'decalog' ),
			'id'      => 'decalog-contextual-settings-loggers-logging',
			'content' => $content . $this->get_loggers( 'logging' ),
		];
		// Monitoring.
		$content = '<p>' . esc_html__( 'These loggers collect metrics', 'decalog' ) . '</p>';
		$tabs[]  = [
			'title'   => esc_html__( 'Monitoring', 'decalog' ),
			'id'      => 'decalog-contextual-settings-loggers-monitoring',
			'content' => $content . $this->get_loggers( 'metrics' ),
		];
		// Tracing.
		$content = '<p>' . esc_html__( 'These loggers capture traces:', 'decalog' ) . '</p>';
		$tabs[]  = [
			'title'   => esc_html__( 'Tracing', 'decalog' ),
	...
	private function set_contextual_viewer_main() {
		...
		if ( false !== strpos( $this->screen->id, 'page_decalog-tviewer' ) ) {
			$content  = '<p>' . esc_html__( 'This screen displays the list of traces belonging to a specific WordPress logger. This list is sorted with the most recent trace at the top.', 'decalog' ) . '</p>';
			$content .= '<p>' . esc_html__( 'To move forward or backward in time, use the navigation buttons at the top or bottom right of this list.', 'decalog' ) . '</p>';
			$content .= '<p>' . esc_html__( 'You can change the traces log being viewed (if you have set more than one WordPress logger) with the selector at the top left of the list (don\'t forget to click on the "apply" button).', 'decalog' ) . '</p>';
			$content .= '<p>' . esc_html__( 'To filter the displayed traces, use the small blue funnel next to the filterable items. These filters are cumulative, you can activate simultaneously several filters.', 'decalog' ) . '<br/>';
			$content .= '<em>' . esc_html__( 'Note these filters are effective even on pseudonymized or obfuscated fields.', 'decalog' ) . '</em></p>';
			$tabs[]   = [
				'title'   => esc_html__( 'Overview', 'decalog' ),
				'id'      => 'decalog-contextual-viewer-main-overview',
	...
	private function set_contextual_viewer_event() {
		...
		$content .= '<p><strong>' . sprintf( esc_html__( '%s backtrace', 'decalog' ), 'PHP' ) . ' / ' . sprintf( esc_html__( '%s backtrace', 'decalog' ), 'WordPress' ) . '</strong> &mdash; ' . esc_html__( 'Two different views of the same information: the backtrace of the call that led to this event.', 'decalog' ) . '</p>';
		$tabs[]   = [
			'title'   => esc_html__( 'Overview', 'decalog' ),
	...
	private function set_contextual_viewer_trace() {
		$tabs = [];
		// Overview.
		$content  = '<p>' . esc_html__( 'This screen displays the details of a specific trace.', 'decalog' ) . ' ' . esc_html__( 'It consists of three boxes which give specific details of the trace:', 'decalog' ) . '</p>';
		$content .= '<p><strong>' . esc_html__( 'Trace', 'decalog' ) . '</strong> &mdash; ' . esc_html__( 'General information about the trace.', 'decalog' ) . '</p>';
		$content .= '<p><strong>WordPress</strong> &mdash; ' . esc_html__( 'User and site where the trace has been captured.', 'decalog' ) . '</p>';
		$content .= '<p><strong>' . esc_html__( 'Spans', 'decalog' ) . '</strong> &mdash; ' . esc_html__( 'Spans composing the trace in a timeline. Note: the alternative grey/white graduations correspond to 250 ms.', 'decalog' ) . '</p>';
		$tabs[]   = [
			'title'   => esc_html__( 'Overview', 'decalog' ),
